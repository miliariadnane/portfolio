...
interface SocialIconsProps {
  className?: string;
  without_buymecoffee?:boolean
}

function SocialIcons(props: SocialIconsProps): React.ReactElement {
  const { className,without_buymecoffee } = props;

  return (
    <div className={clsx('mt-2 flex', className)}>
      {React.Children.toArray(Object.entries(contact.links).filter(el=>{ 
      return without_buymecoffee ? el[0]!=='buymeacoffee':true
      }).map(resolveIcon))}
    </div>
  );
}
